...
description: |
  ...
  the `manifest.json` after a fix has been applied. NOT for creating new skills from scratch, skill strategy design, or updating skill content unrelated to a failed installation.
license: Apache-2.0
metadata:
  version: v1
  publisher: google
...
### 1. Editing SKILL.md without fixing the manifest
**Symptom**: Editing SKILL.md without fixing the manifest
**Problem**: Fixing the skill file content doesn't repair the broken installation if the manifest.json is out of sync.
**Solution**: Always update manifest.json after fixing a SKILL.md to ensure the agent runtime picks up the corrected version.
