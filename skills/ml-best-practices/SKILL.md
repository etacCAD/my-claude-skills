...
description: |
  ...
  SQL/BigQuery ML HANDOFF: If the user requires a SQL solution, use this skill to dictate the ANALYSIS STEPS (e.g., markdown analysis cells, visualization logic), but defer to `bigquery` for all SQL syntax. NOT for building production ML infrastructure, MLOps pipelines, or non-ML data analysis tasks.
license: Apache-2.0
metadata:
  version: v1
  publisher: google
...
### 1. Training on the full dataset before establishing a baseline
**Symptom**: Training on the full dataset before establishing a baseline
**Problem**: Jumping into complex model training without a simple baseline makes it impossible to know if the complexity is helping.
**Solution**: Always establish a baseline (majority class classifier, linear model, or mean predictor) before training anything complex.

### 2. Leaking test data into training via preprocessing
**Symptom**: Leaking test data into training via preprocessing
**Problem**: Fitting scalers, imputers, or encoders on the full dataset before the train/test split introduces data leakage and inflates metrics.
**Solution**: Fit all preprocessing transformers on training data only. Apply (transform only) to validation and test sets.
